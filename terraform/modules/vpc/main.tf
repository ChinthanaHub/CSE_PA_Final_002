...
# Cross-referencing rules are separate resources to avoid a dependency cycle
# between eks_cluster and eks_nodes security groups.
...
resource "aws_security_group_rule" "cluster_to_nodes_kubelet" {
  ...
  security_group_id        = aws_security_group.eks_nodes.id
  source_security_group_id = aws_security_group.eks_cluster.id
  description              = "Allow control plane to nodes on kubelet ports"
}
